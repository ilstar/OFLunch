...
class Admin::MenuItemsController < Admin::BaseController
  ...
  def destroy
    ...
    respond_to do |format|
      ...
      if meal_time = MealTime.opened and (meal_time.activated? or meal_time.locked?) and (today_order_items = @menu_item.order_items.where(["created_at > ?", Time.current.beginning_of_day])).count > 0
